'''
smacs3/rrRcontr.py <- smacs2/rrcontr.py  (history: smacs1/acontr.py)
...
source and sink are remote via 0MQ ipc
u-plane is provided by RabbitMQ instead of 0MQ in /smacs2 and /newtwork, reflected in CONF

7/20/2023/nj, laste update 8/15/2023
'''
   ...
   #packet for mode 1 and 3 
    ...
    def clt_handler(self, rcvd: dict)->dict:
        match self.conf['mode']:
            ...
            case _: 
                print('mode unknown in Controller:', self.conf['mode'])
                exit()
    ...
    def mode0(self, rcv_data:dict)->dict:
        ...
        if ccdu['seq'] > self.state['seq'] and pcdu['seq'] > self.state['seq'] and ccdu['seq'] == pcdu['seq']:
            ...
            if self.state['sent']:                      #second step of pause: last update, to avoid intial state
                ...
                cdu = self.cdu0(self.state['seq']+1, dict()) #self.state['sent'] = False              #is sent 
                return cdu 
            else:                                       #not sent yet
        ...
        if pcdu['proto'] == 1 and ccdu['proto'] == 1:
            if ccdu['mseq'] > self.state['mseq'] and pcdu['mseq'] > self.state['mseq'] and ccdu['mseq'] == pcdu['mseq']:
                ...
                if self.met():                                          #self.state['met'] filled with new
                    ...
                    if self.state['mseq'] > self.conf['cnt']: 
                        ...
                        cdu = self.cdu3(self.state['seq']+1, self.state['mseq'], 0)         #deliver met
                        self.state['met'].clear()
                    else:
                        cdu = self.cdu3(self.state['seq'], self.state['mseq']+1, self.proto)        #next measurement 
                    return cdu

            cdu = self.cdu3(self.state['seq'], self.state['mseq']+1, self.proto)        #retransmit of last measurement
            ...
            if ccdu['seq'] > self.state['seq'] and pcdu['seq'] > self.state['seq'] and ccdu['seq'] == pcdu['seq']:
                ...
                if self.state['crst']:
                    ...
                    cdu = self.cdu3(self.state['seq']+1, self.state['mseq'], 0)     #next message 
                return cdu
            else:
                cdu = self.cdu3(self.state['seq']+1, self.state['mseq'], 0)         #retransmit for last message
        else:
...
def add_data(col, tag, entry):
    ...
    if doc and isinstance(doc['data'], list):
        ...
        print('saved with', rst.inserted_id, rst.acknowledged)
        return True 
    return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print('usage: python3 rrRcontr.py (mode 0, 1,2,3')
        print('usage: python3 rrRcontr.py -testdb(test db)/prepdb(prepare db)/clt(test req-rep)')
        exit()
