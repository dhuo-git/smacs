"""
smacs3/rrRprod.py <- smacs2/rrprod.py (history: smacs1/aprod.py,apubsub.py)
...
class rabbitRpc.RpcServer: u-plane is based on RabbitMQ via Rcons.py

child class SaveProd(Slave, Prod):
    provides mode 0,1,3 support to SMACS
sends on N5 interface and receive on N0
created 7/20/2023,  last update 8/15/2023
"""
...
class SlaveProd(Slave, Prod):
    ...
    def pst_template(self):
        ...
        p2c= {'rtkey': self.conf['rtkey'],'seq':0,'mseq':0,  'pt':[],'urst': False, 'update': False, 'proto': 1}
