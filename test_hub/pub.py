...
class Pub:
    ...
    def publisher(self): #context = zmq.Context()
        print('server {} publishes from port {} on topics {}'.format(self.conf['pub_id'], self.conf['pub_port'], self.conf['pubtopics'])) 
        while self.pub_active:
            for topic in self.conf['pubtopics']:
                payload = self.pub_handler(self.conf['pub_id'], topic) 
                message={'sdu': payload, 'topic':topic, 'issuer':self.conf['pub_id']}
    ...
    def pub_handler(self, publisher_id, topic):
        tx = {'pid': publisher_id,'top': topic, 'seq':self.seq}# time.ctime()}
        self.seq += 1
        if self.conf['buffer']: self.buffer.append(tx)
        else: print('Server sends', tx)
        return tx

    def pubtest(self):
        for topic in self.conf['pubtopics']:
            payload = self.test_handler(self.conf['pub_id'], topic) 
            message={'sdu': payload, 'topic':topic, 'issuer':self.conf['pub_id']}
            bstring = json.dumps(message)
            self.socket.send_string("%d %s"% (topic, bstring)) 
        print('server {} publishes from port {} on topics {}'.format(self.conf['pub_id'], self.conf['pub_port'], self.conf['pubtopics'])) 
        if topic == self.conf['pubtopics'][-1]:
            ...
            print('server {} cannot publish from port {} on topics {}'.format(self.conf['pub_id'], self.conf['pub_port'], self.conf['pubtopics'])) 
            return False 
    def test_handler(self, publisher_id, topic):
        tx = {'pid': publisher_id,'top': topic, 'seq': self.seq,'tm': time.ctime()}
        self.seq += 1 
        if self.conf['buffer']:
...
CONF = {'ipv4':'127.0.0.1' , 'pub_port': "5568", 'pubtopics':[0,1,2,3,4], 'pub_id':1,'dly':1., 'buffer': False}
