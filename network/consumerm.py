...
'''
consumer.py, similiar to producer.py, consists of a Sub and a Pub:
    Sub(sub.py): receives data from sub_deque for 'subtopics'
    Pub(pub.py): sends data to pub_queue for 'pubtopics'

'''
from threading import Thread
import subm, pubm
#receiver subscribe to sender via medium 
SubS_CONF = {'ipv4':"127.0.0.1" , 'sub_port': "5570", 'subtopics':[6], 'sub_id':3, 'dly':1., 'name': 'Consumer', 'tstmp': True, 'maxlen': 4,  'print': True}
#receiver publisher states for controler
PubC_CONF = {'ipv4':"127.0.0.1" , 'pub_port': "5568", 'pubtopics':[106], 'pub_id':3, 'dly':1., 'name': 'Consumer', 'tstmp': True, 'maxlen': 10, 'sdu': {'seq':0}, 'print': True}


#three independent channels: receive traffic(sub), receive control(sub), send states(pub)
if __name__=="__main__":
    inst =[ subm.Sub(SubS_CONF),  pubm.Pub(PubC_CONF)]
