...
compared to network/controller, REQ-REP is deployed for N0/5/7, where controller is client and uses REQ
...
class Controller:
    ...
    def open(self):
        ...
        self.preq_socket = self.context.socket(zmq.REQ) #N5
        self.preq_socket.connect(f"tcp://{self.conf['ipv4p']}:{self.conf['rep_p']}")
 
        self.creq_socket = self.context.socket(zmq.REQ) #N7
        self.creq_socket.connect(f"tcp://{self.conf['ipv4c']}:{self.conf['rep_c']}")

        self.tmp = copy.deepcopy(self.conf)#self.template_tmp(self.conf['conf']) print(self.tmp)
   ...
   #packet for mode 1 and 3 
    ...
    #transmit via REQ-REP as multi-cast
    def requestTx(self, rcdu, note, sdu = dict()):
        message = {'cdu': copy.deepcopy(rcdu), 'sdu': copy.deepcopy(sdu)}
        bstring = json.dumps(message)
        #self.preq_socket.send(bstring)
        #self.creq_socket.send(bstring)
        self.preq_socket.send_string(bstring)
        self.creq_socket.send_string(bstring)
        print(note, rcdu)
    #receive via REQ-REP
    def requestRx(self, note, i=None):
        if i=='p':
            bstring = self.preq_socket.recv()
        elif i=='c':
            bstring = self.creq_socket.recv()
        else:
            print('unknown socket in repond', i)
            exit()

        message = json.loads(bstring)
        cdu = message['cdu']
        print(note, message)
    #---Mode Test
    def TestTx(self):
    ...
    def Mode0Loop(self):
        ...
        while self.state['loop']:
            if not (self.state['ack'][0] and self.state['ack'][1]):
                #need async
                p_cdu = self.requestRx('rx:', 'p')
                if p_cdu['seq'] > self.state['seq']: 
                    self.state['tseq'][0] = p_cdu['seq']
                    self.state['ack'][0] = True
                c_cdu = self.requestRx('rx:', 'c')
                if c_cdu['seq'] > self.state['seq']: 
                    self.state['tseq'][1] = c_cdu['seq']
                    self.state['ack'][1] = True
            elif self.state['tseq'][0] == self.state['tseq'][1]:# and self.state['tseq'][0] > self.state['seq']: 
    ...
    def Mode1Rx(self):
        ...
        while self.state['loop']: #True: 
            ...
            #self.close()#exit()#self.state['loop'] = False
            return 
...
P_CONF = {'ipv4':ipv4, 'sub_port': "5570", 'pub_port': "5568", 'key':[1, 2], 'ipv4p':ipv4p,'rep_p': 5550,  'dly':1., 'maxlen': 4, 'print': True, 'mode': 0}
P_CONF.update({'ctr_sub': 0, 'ctr_pub': 5, 'u_sub': 6, 'u_pub': 4})
C_CONF = {'ipv4':ipv4 , 'sub_port': "5570", 'pub_port': "5568", 'key':[1,2], 'ipv4c':ipv4c, 'rep_p': 5555, 'dly':1., 'maxlen': 4,  'print': True, 'mode': 0}
C_CONF.update({'ctr_sub': 0, 'ctr_pub': 7, 'u_sub': 4, 'u_pub': 6})

#from producer import CONF as P_CONF
#from consumer import CONF as C_CONF

CONF = {"ipv4":ipv4 , "sub_port": "5570", "pub_port": "5568", "ipv4p": ipv4p, "ipv4c": ipv4c,"rep_p": 5550, "rep_c": 5555, "id":0, "dly":0, "print": True, "ver": 0,  'cnt':20, "mode":0, "uperiod": 0}
