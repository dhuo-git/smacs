'''
acontrtest.py (for testing acontr.py ) <-smacs1/acontr.py
...
'''
import asyncio
import time, sys,json, pprint, copy
...
class Master:
    ...
    def client(self):
        async def timed_concurrent():
            timer = 10
            rcv_data = []
            message= self.clt_handler(rcv_data)
            #breakpoint()
            while self.loop:
                try:
                    coros = [tcp_client(message, ip, port) for ip, port in self.svrs]
                    rcv_data = await asyncio.wait_for(asyncio.gather(*coros), timer, )
                except asyncio.TimeoutError:
                    print("Time over")
                finally: #print('cors', coros) #print('rcv data', rcv_data)
                    message= self.clt_handler(rcv_data)
        # is used by both concurrent or timed_concurrent
        async def tcp_client(message: dict, ip:str, port:int)->dict: 
            ...
            writer.write(mstring.encode())
            await writer.drain()
            #receive
            data = await reader.read(1024)
            writer.close()
            await writer.wait_closed()
            rcvd = json.loads(data.decode())
            rcvd['pt'].append(time.time_ns())
            rcvd['ct'].append(time.time_ns())
            return rcvd
        try:
            asyncio.run(timed_concurrent())
        except RuntimeError:
            print('client failed')
