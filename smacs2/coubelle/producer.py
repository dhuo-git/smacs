...
compared to network/producer.py, REQ-REP is deployed for N0/N5, where producer is server with REP
...
class Producer:
    ...
    def open(self):
        ...
        #self.req_socket = self.context.socket(zmq.REQ) #N5
        self.rep_socket = self.context.socket(zmq.REP) #N5
        self.rep_socket.bind(f"tcp://{self.conf['ipv4p']}:{self.conf['rep_p']}")
    ...
    #transmit via REQ-REP
    def respondTx(self, rcdu, note, sdu = dict()):
        message = {'cdu': copy.deepcopy(rcdu), 'sdu': copy.deepcopy(sdu)}
        bstring = json.dumps(message)
        self.rep_socket.send(bstring)
        print(note, rcdu)
    #receive via REQ-REP
    def respondRx(self, note):
        bstring = self.rep_socket.recv()
...
#ipv4= "192.168.1.37"    #
ipv4p="192.168.1.37"
CONF = {'ipv4':ipv4, 'sub_port': "5570", 'pub_port': "5568","ipv4p":ipv4p, "rep_p": 5550, 'key':[1, 2], 'dly':1., 'maxlen': 4, 'print': True, 'mode':0}
