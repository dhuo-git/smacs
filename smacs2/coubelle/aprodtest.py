...
class Slave:
    ...
    def server(self):
        async def handle_client(reader, writer): 
            ...
            mstring = data.decode()
            try:                                    #attach time stamp for received packet
                message = json.loads(mstring)       #receive from N0
            ...
            response =  self.produce(message)       #send to N5
            response['ct'].append(time.time_ns())   #attach time stamp for transmitted packet

            data = json.dumps(response)
            writer.write(data.encode())
            await writer.drain() #print("Close the connection")
            writer.close()
            await writer.wait_closed()              #safe close
            """ TEST """
...
if __name__ == '__main__':
    ...
    from acontr import P_CONF as N4CONF
